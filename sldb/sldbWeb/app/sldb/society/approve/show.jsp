...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script type="text/javascript">
			...
			$('#queryTable').datagrid({
				...
				toolbar : [{
					...
					handler : function() {
						...
						window.location.href='<%=request.getContextPath()%>/app/sldb/society/query2.action?id='+ rows[0].id +'&view=edit';
						return false;
					}
				}, 	{
					id : 'btnItem',
		...
		function confirmVO(id){
			$.messager.confirm('确认审批', '确认审批该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/society/approve/pass.jsp?ids=' + id;
				}
			});
			return false;
		}
		...
		function refuseVO(id){
			$.messager.confirm('确认驳回', '确认驳回该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/society/audit/refuse.jsp?ids=' + id +'&type=5';
				}
			});
			return false;
		}
		...
		<form id="queryForm" method="post">
			<table>
				<tr align="right">
					...
					<td><input id="state" name="state" type="hidden" value="待审批"></input></td>
				</tr><tr>
				</tr>
			</table>
