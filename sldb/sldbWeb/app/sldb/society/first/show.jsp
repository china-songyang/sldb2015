...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script type="text/javascript">
		...
		function confirmVO(id){
			$.messager.confirm('确认初审', '确认初审该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/society/first/pass.jsp?ids=' + id;
				}
			});
			return false;
		}
		...
		function refuseVO(id){
			$.messager.confirm('确认驳回', '确认驳回该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/society/audit/refuse.jsp?ids=' + id +'&type=3';
				}
			});
			return false;
		}
		...
		<form id="queryForm" method="post">
			<table>
				<tr align="right">
					...
					<td><input id="state" name="state" type="hidden" value="待初审"></input></td>
				</tr><tr>
				</tr>
			</table>
