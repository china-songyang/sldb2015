	...
	<script type="text/javascript">
			...
			$('#queryTable').datagrid({
				...
				toolbar : [{
					...
					handler : function() {
						...
						window.location.href='<%=request.getContextPath()%>/app/sldb/society/check/check.jsp?pid='+rows[0].id;
						return false;
					}
				},
				{
		...
		function confirmVO(id){
			$.messager.confirm('确认核对', '确认核对该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/society/check/pass.jsp?ids=' + id;
				}
			});
			return false;
		}
		...
		function refuseVO(id){
			$.messager.confirm('确认驳回', '确认驳回该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/sldb/society/audit/refuse.jsp?ids=' + id +'&type=2';
				}
			});
			return false;
		}
