...
public class IdCheck {
	...
	public static int add(IdCheck vo) {
		...
		return SimpleDaoTemplate
				.update("INSERT INTO sldb_id_check (`id`, `key`, `result`, `infoType`,`value`,`checkTime`) VALUES(?, ?, ?, ?, ?, ?)",
						vo.id, vo.key, vo.result, vo.infoType, vo.value, vo.checkTime);
	}
	...
	public static String checkSociety(String pid) {
		Society person = Society.get(pid);
		List<Sub> subs = Sub.getByPid(pid);
	...
	public static void checkOnePerson(String pid, String identify, String name) {
		...
		String checkTime = DateUtils.getSystemDate();
		...
		for (ImportInfo info : infos) {
			...
			if (importLast != null) {
				...
				if (results.isEmpty()) {
					add(new IdCheck(id, key, "无", info.getId(), "", checkTime));
				} else {
					List<IdCheck> checkResult = getList(results, id, key,
							info.getId(), checkTime);
	...
	//根据姓名进行核对
		public static String checkSocietyByName(String pid) {
			Society person = Society.get(pid);
			List<Sub> subs = Sub.getByPid(pid);
	...
	/*根据身份证号和时间核对数据  --sy120613*/
	public static String checkByIdWithDate(String pid, String createTime) {
		Person person = Person.get(pid);  //根据ID获取户主信息
		List<PersonSub> subs = PersonSub.getByPid(pid);  //根据ID获取子女信息

		checkByDate(pid, person.getIdentify(), createTime);
	...
	public static void checkByDate(String pid, String identify, String createTime) {
		List<IdCheck> checks = new ArrayList<IdCheck>();
		List<ImportInfo> infos = ImportInfo.getAllList();
		...
		for (ImportInfo info : infos) {
			Import importLast = getLastImportWithTime(info.getId(),createTime);
	...
	public static Import getLastImportWithTime(String type,String createTime) {
		List<Import> imports = SimpleDaoTemplate.query(
				"select * from sldb_import where type = '" + type
						+ "' and createTime <= '" + createTime + "'order by importDate desc", null, mappingImport,
				Import.class);
	...
	//根据身份证号核对数据
	public static String checkSocietyByIdentify(String pid) {
		Society person = Society.get(pid);
		List<Sub> subs = Sub.getByPid(pid);
